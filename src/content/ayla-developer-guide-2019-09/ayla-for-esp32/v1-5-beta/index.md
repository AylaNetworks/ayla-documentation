---
title: Ayla for ESP32 v1.5-beta
...
1. [Create a template](../../common-tasks/create-a-template) with the following properties and details:
    ...
    1. Details:
        ...
        * Visibility: ```oem```
        * Name: ```ADA ESP 1.5 Beta```
        * Description: ```ADA ESP 1.5 Beta```
        * Registration: ```Dsn```
        * Model: ```ledevb```
        * Version: ```ada-esp-idf-src-1.5-beta```
...
1. Install Ayla source code:
    ...
    1. Copy the ```ada-esp-idf-src-1.5-beta``` directory on top of the ```esp-idf``` directory:

        ```
      # cp -R ada-esp-idf-src-1.5-beta/* ./esp-idf
        ```
